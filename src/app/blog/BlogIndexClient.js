...
/** Normalize blog image path for Next/Image (supports /images/ and images/) */
function blogImageSrc(path) {
  if (!path) return "";
  return path.startsWith("/") ? path : `/${path}`;
}

export default function BlogIndexClient({ initialBlogs, categories }) {
          ...
          <div className="bi-masthead-inner">
            ...
            <div className="bi-stats-row">
              <div className="bi-stat">
                <span className="bi-stat-num">{initialBlogs?.length ?? 0}</span>
                <span className="bi-stat-label">Articles</span>
              </div>
              <div className="bi-stat">
                <span className="bi-stat-num">{(categories?.length ?? 1) - 1}</span>
              ...
              {featured && (
                <Link href={`/blog/${featured.slug}`} className="bi-featured">
                  <div className="bi-featured-img-wrap">
                    <Image src={blogImageSrc(featured.image)} alt={featured.title} fill sizes="(max-width: 768px) 100vw, 55vw" className="object-cover" />
                    ...
                    {rest.map((post) => (
                      <Link key={post.slug} href={`/blog/${post.slug}`} className="bi-card">
                        <div className="bi-card-img-wrap">
                          <Image src={blogImageSrc(post.image)} alt={post.title} fill sizes="(max-width: 1024px) 50vw, 33vw" className="object-cover" />
